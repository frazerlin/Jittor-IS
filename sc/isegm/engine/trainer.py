...
def get_sampler(dataset, shuffle, distributed):
    if shuffle:
        return jittor.dataset.RandomSampler(dataset)
    else:
        return jittor.dataset.SequentialSampler(dataset)
...
class ISTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with jittor.enable_grad():
            ...
            prev_output = jittor.zeros_like(image, dtype=jittor.float32)[:, :1, :, :]
            ...
            with jittor.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    net_input = jittor.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image
                    prev_output = jittor.sigmoid(eval_model(net_input, points)['instances'])
                ...
                if self.net.with_prev_mask and self.prev_mask_drop_prob > 0 and last_click_indx is not None:
                    ...
                    prev_output[zero_mask] = jittor.zeros_like(prev_output[zero_mask])
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        predicted_instance_masks = jittor.sigmoid(outputs['instances']).detach().cpu().numpy()
...
def load_weights(model, path_to_weights):
    ...
    new_state_dict = jittor.load(path_to_weights, map_location='cpu')['state_dict']
