...
class MixFFN(BaseModule):
    ...
    def __init__(self,
        ...
        self.dropout_layer = build_dropout(
            dropout_layer) if dropout_layer else jittor.nn.Identity()

    def execute(self, x, hw_shape, identity=None):
...
class MixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        dpr = [
            x.item()
            for x in jittor.linspace(0, drop_path_rate, sum(num_layers))
    ...
    def init_weights(self):
        if self.pretrained is None:
            ...
            state_dict['layers.0.0.projection.weight'] = jittor.cat([ori_proj_weight, ori_proj_weight], dim=1)

            self.load_state_dict(state_dict, True)


    def execute(self, x, additional_features=None):
...
class SegformerHead(BaseDecodeHead):
    ...
    def execute(self, inputs):
        ...
        out = self.fusion_conv(jittor.cat(outs, dim=1))
