...
class BaseOptimizer:
    ...
    def __call__(self, x):
        opt_params = jittor.from_numpy(x).float().to(self.device)
        ...
        with jittor.enable_grad():
            ...
            result = jittor.sigmoid(result_before_sigmoid)

            pos_mask, neg_mask = self._click_masks
            if self.with_flip and self.flip_average:
                result, result_flipped = jittor.chunk(result, 2, dim=0)
                result = 0.5 * (result + jittor.flip(result_flipped, dims=[3]))
...
class InputOptimizer(BaseOptimizer):
    def unpack_opt_params(self, opt_params):
        ...
        if self.with_flip:
            opt_params_flipped = jittor.flip(opt_params, dims=[3])
            opt_params = jittor.cat([opt_params, opt_params_flipped], dim=0)
        reg_loss = self.reg_weight * jittor.sum(opt_params**2)
...
class ScaleBiasOptimizer(BaseOptimizer):
    ...
    def unpack_opt_params(self, opt_params):
        scale, bias = jittor.chunk(opt_params, 2, dim=0)
        reg_loss = self.reg_weight * (jittor.sum(scale**2) + self.reg_bias_weight * jittor.sum(bias**2))

        if self.scale_act == 'tanh':
            scale = jittor.tanh(scale)
        elif self.scale_act == 'sin':
            scale = jittor.sin(scale)
