...
class NormalizedFocalLossSigmoid(nn.Module):
    ...
    def execute(self, pred, label):
        ...
        # sw_sum = jittor.sum(sample_weight, dim=-2, keepdims=True)
        # sw_sum = jittor.sum(sw_sum, dim=-1, keepdims=True)
        beta_sum = jittor.sum(beta, dims=(-2, -1), keepdims=True)
        # beta_sum = jittor.sum(beta, dim=-2, keepdims=True)
        # beta_sum = jittor.sum(beta, dim=-1, keepdims=True)
