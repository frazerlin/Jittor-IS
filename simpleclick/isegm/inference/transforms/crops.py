...
class Crops(BaseTransform):
    ...
    def transform(self, image_nd, clicks_lists: List[List[Click]]):
        ...
        image_crops = jittor.cat(image_crops, dim=0)
        self._counts = jittor.tensor(self._counts, device=image_nd.device, dtype=jittor.float32)
    ...
    def inv_transform(self, prob_map):
        ...
        new_prob_map = jittor.zeros((1, 1, *self._counts.shape),
        ...
        for dy in self.y_offsets:
            for dx in self.x_offsets:
                ...
                crop_indx += 1
        new_prob_map = jittor.div(new_prob_map, self._counts)
