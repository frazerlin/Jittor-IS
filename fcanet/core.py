...
class ToTensor(object):
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            ...
            tmp = jittor.array(tmp).float32()
            tmp = tmp.float32().divide(255) if self.if_div else tmp
            sample[elem] = tmp                          
        return sample
...
def init_model(model_name='fcanet',backbone='resnet',pretrained_file=None, if_cuda=True):
    ...
    if pretrained_file is not None:
        if if_cuda:
            state_dict=jittor.load(pretrained_file)
        else:
            state_dict=jittor.load(pretrained_file,map_location=lambda storage, loc: storage)
...
def predict(model,img, seq_points,  if_sis=False, if_cuda=True):
    ...
    with jittor.no_grad(): 
        output = model(input)
    # result = jittor.sigmoid(output.data.cpu()).numpy()[0,0,:,:]
    result = jittor.sigmoid(output.data).numpy()[0,0,:,:]
