...
def get_mask_gauss(mask_dist_src, sigma):
    ...
    return jittor.exp(-2.772588722*(mask_dist_src**2)/(sigma**2))
...
class MyASPP(nn.Module):
    ...
    def execute(self, x):
        ...
        if self.if_global:
            ...
            x = jittor.cat((x1, x2, x3, x4, x5), dim=1)
        else:
            # x = torch.cat((x1, x2, x3, x4), dim=1)
            x = jittor.cat((x1, x2, x3, x4), dim=1)
...
class MyDecoder(nn.Module):
    ...
    def execute(self, input, side):
        ...
        merge=jittor.cat((input, side), dim=1)
...
class FCANet(nn.Module):
    ...
    def execute(self, input):
        ...
        img_with_anno = jittor.cat((img, pos_mask_gauss, neg_mask_gauss), dim=1)
        ...
        l1_first=jittor.cat((l1, nn.interpolate(pos_mask_gauss_first, size=l1.size()[2:], mode='bilinear', align_corners=True)),dim=1)
        l1_first=self.first_conv(l1_first)
        # l4=torch.cat((l1_first,l4),dim=1)
        l4=jittor.cat((l1_first,l4),dim=1)
        x=self.my_aspp(l4)
        x=self.my_decoder(x,l1)
        x=self.pred_decoder(x)
        # result = F.interpolate(x, size=img.size()[2:], mode='bilinear', align_corners=True)
        result = nn.interpolate(x, size=img.size()[2:], mode='bilinear', align_corners=True)
        return result
