...
class Bottle2neck(nn.Module):
    ...
    def execute(self, x):
        ...
        spx = jittor.split(out, self.width, 1)
        for i in range(self.nums):
          ...
          if i==0:
            ...
            out = jittor.cat((out, sp), 1)
            # out = torch.cat((out, sp), 1)
        if self.scale != 1 and self.stype=='normal':
        #   out = torch.cat((out, spx[self.nums]),1)
          out = jittor.cat((out, spx[self.nums]),1)
        elif self.stride==1 and self.scale != 1 and self.stype=='stage':
        #   out = torch.cat((out, spx[self.nums]),1)
          out = jittor.cat((out, spx[self.nums]),1)
        elif self.scale != 1 and self.stype=='stage':
        #   out = torch.cat((out, self.pool(spx[self.nums])),1)
          out = jittor.cat((out, self.pool(spx[self.nums])),1)
...
class Res2NetSplit(nn.Module):
    ...
    def execute(self, input, if_final=True):
        x=input
        results=[]
