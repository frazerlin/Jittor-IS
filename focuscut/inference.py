...
def predict_wo(p,model,sample,seq_points,mode='eval'):
    ...
    sample_batched=collate_batch([sample_cpy])
    ...
    for turn in range(turn_num):
        if len(hr_points)>0:
            ...
            if len(sample_hrs)>0:
                ...
                sample_batched_hr=collate_batch(sample_hrs)
                with jittor.no_grad(): output = model(sample_batched_hr,mode='eval-hr')
                ...
                for hr_result, sample_hr in zip(hr_results,sample_hrs):
                    ...
                    crop_bbox=(pt_hr[0],pt_hr[1],2*expand_r+1,2*expand_r+1)
                    # print(type(hr_result))
                    hr_result=hr_result.numpy()
