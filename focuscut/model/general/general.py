...
# import jittor.nn.functional as nn
import jittor
import jittor.nn as nn
...
def init_weight(model):
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            jittor.nn.init.kaiming_normal_(m.weight)
...
def gene_map_gauss(map_dist_src, sigma=10):
    return jittor.exp(-2.772588722*(map_dist_src**2)/(sigma**2))

def gene_map_dist(map_dist_src, max_dist=255):
    return 1.0-map_dist_src/max_dist

def my_resize(input,ref):
    return nn.interpolate(input, size=(ref if isinstance(ref,(list,tuple)) else ref.size()[2:]), mode='bilinear', align_corners=True)
...
def point_resize(mask,ref,if_return_mask=True,tsh=0.5):
    ...
    indices=jittor.nonzero(mask>tsh)
    if mask_h==ref_h and mask_w==ref_w:
        mask_new= mask
    else:
        mask_new=jittor.zeros([mask.shape[0],mask.shape[1],ref_h,ref_w])
        if len(indices)!=0:
            indices = indices.float()
            indices[:,2] = jittor.floor(indices[:,2]*ref_h/mask_h)
            indices[:,3] = jittor.floor(indices[:,3]*ref_w/mask_w)
...
def get_click_map(sample_batched,mode='dist',click_keys=None):
    ...
    if mode.startswith('dist'):
        ...
        click_map = jittor.concat([(gene_map_dist(t,int(para)) if if_with_para else gene_map_dist(t)) for t in get_key_sample(sample_batched,click_keys,if_list=True)],dim=1)
    elif mode.startswith('gauss'):
        if click_keys is None: click_keys=['pos_map_dist_src','neg_map_dist_src']
        click_map = jittor.concat([(gene_map_gauss(t,int(para)) if if_with_para else gene_map_gauss(t)) for t in get_key_sample(sample_batched,click_keys,if_list=True)],dim=1)
    elif mode=='point':
        if click_keys is None: click_keys=['pos_points_mask','neg_points_mask']
        click_map = jittor.concat(get_key_sample(sample_batched,click_keys,if_list=True), dim=1)
...
def get_input(sample_batched,mode='dist'):
    ...
    if mode in ['dist','gauss','point']:
        x= jittor.concat((img, get_click_map(sample_batched,mode)),dim=1)
...
def get_stack_flip_feat(x,if_horizontal=True):
    x_flip= jittor.flip(x,[3 if if_horizontal else 2]) 
    return jittor.concat([x,x_flip],dim=0)

def merge_stack_flip_result(x,if_horizontal=True,batch_num=1):
    return (x[:batch_num]+ jittor.flip(x[batch_num:2*batch_num],[3 if if_horizontal else 2]))/2.0
            ...
            if block_kind=='conv':
                if channels[i]==1 or if_end_wo_relu:
                    ...
                    if if_w_bn:
                        ...
                        convs_tmp.append(nn.Sequential(nn.Conv2d( in_ch if i==0 else channels[i-1] , channels[i], kernel_size=kernel_sizes[i], stride=strides[i], padding=paddings[i], dilation=dilations[i],bias=False), nn.ReLU()))
...
class MyASPP(nn.Module):
    ...
    def execute(self, x):
        ...
        if self.if_global:
            ...
            x5 = nn.interpolate(x5, size=x4.size()[2:], mode='bilinear', align_corners=True)
            x = jittor.concat((x1, x2, x3, x4, x5), dim=1)
        else:
            x = jittor.concat((x1, x2, x3, x4), dim=1)
...
class MyDecoder(nn.Module):
    ...
    def execute(self, input, side):
        ...
        if self.size_ref=='side':
            input=nn.interpolate(input, size=side.size()[2:], mode='bilinear', align_corners=True)
        elif self.size_ref=='input':
            side=nn.interpolate(side, size=input.size()[2:], mode='bilinear', align_corners=True)

        merge=jittor.concat((input, side), dim=1)
...
class PredDecoder(nn.Module):
    def __init__(self,in_ch,layer_num=1,BatchNorm=nn.BatchNorm2d,if_sigmoid=False):
        ...
        for i in range(layer_num-1):
            convs_tmp.append(nn.Sequential(nn.Conv2d(in_ch,in_ch//2,kernel_size=3,stride=1,padding=1,bias=False), BatchNorm(in_ch//2), nn.ReLU()))
            in_ch=in_ch//2
        ...
        init_weight(self)
        # print(self.pred_conv[0].weight.numpy())
        # print('-----------')
...
class MyNetBase(nn.Module):
    ...
    def get_loss(self, output, gt=None,sample_batched=None,others=None):
        ...
        losses = [nn.binary_cross_entropy_with_logits(my_resize(t,gt),gt) for t in make_list(output)]
        return losses
    ...
    def get_loss_union(self,output,gt=None,sample_batched=None,others=None):
        ...
        loss_items=jittor.array([loss.item() for loss in losses]).unsqueeze(0)
        losses=sum(losses)
        losses.backward()
        return loss_items

    def get_result(self, output, index=None):
        result = jittor.sigmoid((make_list(output))[0]).data
        return result[:,0,:,:] if index is None else result[index,0,:,:]

    def execute_union(self, sample_batched, mode='train'):
        output=self.execute(sample_batched,mode)
        if mode=='train':
...
class MyNetBaseHR(MyNetBase):
    def __init__(self,input_channel=5, output_stride=16, if_sync_bn=False, if_freeze_bn=False, special_lr=0.1,remain_lr=1.0,size=512,aux_parameter={}):
        ...
        # self.backbone = resnet.get_resnet_backbone(self.aux_parameter['backbone'],output_stride,BatchNorm,self.aux_parameter['pretrained'],3,True)
        ...
        if self.aux_parameter['into_layer']!=-1:
            ...
            self.encoder_anno=nn.Sequential( nn.Conv2d(in_ch+2+int(self.aux_parameter['if_pre_pred']),in_ch , 1, bias=False), BatchNorm(in_ch), nn.ReLU())
    ...
    def backbone_execute(self,sample_batched,img_key,click_keys,pre_pred_key,return_list=['final']):
        ...
        aux= jittor.concat([click_map,pre_pred],dim=1)  if self.aux_parameter['if_pre_pred'] else click_map
        ...
        # print(x.shape)
        if self.aux_parameter['into_layer']==-1:
            x=jittor.concat((x,my_resize(aux,x)),dim=1)
        for i in range(5):
            if i==1:x=self.backbone.layers[0][-1](x)

            if self.aux_parameter['into_layer']==i:
                x=self.encoder_anno(jittor.concat((x,my_resize(aux,x)),dim=1))

            # x=self.backbone.layers[i][:-1](x) if i==0 else self.backbone.layers[i](x)

            if i == 0:
                for layer in self.backbone.layers[i][:-1]:
                    x = layer(x)
                    # print(layer.weight.cpu().detach().numpy())
                #     print(layer)
                #     print('i0: ',i, x.detach().cpu().numpy().sum(),x.detach().cpu().numpy().mean())
                #     print(x[0,:3,:3,0].detach().cpu().numpy())
                # exit(0)
            else:
        ...
        # print('aspp',x.detach().cpu().numpy().sum())

        x=self.my_decoder(x,l1)
        if return_results_func('decoder',x): return return_results 
        # print('decoder',x.detach().cpu().numpy().sum())

        x=self.pred_decoder(x)
        if return_results_func('pred_decoder',x): return return_results
        # print('pred',x.detach().cpu().numpy().sum())
        # print(self.pred_decoder.pred_conv)
        # print(self.pred_decoder.pred_conv[0].weight.numpy())
        # print(self.pred_decoder.pred_conv[0].weight.shape)
        # exit()

        x=my_resize(x, img)
        # print(x.detach().cpu().numpy().sum())
        # exit(0)
        if return_results_func('final',x): return return_results
        

    def wo_execute(self,sample_batched):
        return self.backbone_execute(sample_batched,'img',['pos_map_dist_src','neg_map_dist_src'],'pre_pred')[0]

    def hr_execute(self,sample_batched):
        return self.backbone_execute(sample_batched,'img_hr',['pos_map_dist_src_hr','neg_map_dist_src_hr'],'pre_pred_hr')[0]

    def execute(self, sample_batched, mode='train'):
        ...
        for part in ['wo','hr']:
            if mode in ['train','eval-{}'.format(part)]:
                ...
                result_part=getattr(self,'{}_execute'.format(part))(sample_batched)
                # print(part)
                # print(result_part)
                ...
                # print(self.backbone.layers[0][0].weight.detach().cpu().numpy().sum())
                # optim=nn.SGD(self.parameters(), 0.001)
                # output_wo=result_part
                # gt=sample_batched['gt'].cuda()
                # wo_loss=nn.binary_cross_entropy_with_logits(output_wo,gt)
        ...
        if mode in ['train']:
            ...
            loss_items=jittor.array([loss.item() for loss in losses]).unsqueeze(0).cuda()
            ...
            ##############################
                ...
                # losses_sum.backward()
            return results,loss_items, losses_sum
        else:
            return results
    ...
    def get_hr_loss(self,output_hr,sample_batched):
        ...
        hr_loss=nn.binary_cross_entropy_with_logits(output_hr,gt_hr,weight=weight)
        return hr_loss
