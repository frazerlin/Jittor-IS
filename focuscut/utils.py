...
def get_optimizer(opt_info, train_params):
    ...
    # opt_kwargs['lr'] = train_params[0]['lr']
    opt_kwargs['lr'] = 0.0
    ...
    if opt_mode=='SGD':
        ...
        for i in range(len(train_params)):
            train_params[i]["params"]=list(train_params[i]["params"])

        # print(train_params)

        return  jittor.optim.SGD(train_params,**opt_kwargs)
    elif opt_mode=='Adam':
        # return torch.optim.Adam(train_params,**opt_kwargs)
        return jittor.optim.Adam(train_params,**opt_kwargs)
...
def get_lr_scheduler(ls_info, optimizer):
    ...
    if ls_mode=='poly':
        ...
        return  jittor.lr_scheduler.StepLR(optimizer, **ls_kwargs)
    elif ls_mode=='multistep':
        # return  torch.optim.lr_scheduler.MultiStepLR(optimizer, **ls_kwargs)
        return  jittor.lr_scheduler.MultiStepLR(optimizer, **ls_kwargs)
    elif ls_mode=='exp':
        # import torch
        # return torch.optim.lr_scheduler.ExponentialLR(optimizer,**ls_kwargs)
        return jittor.lr_scheduler.ExponentialLR(optimizer,**ls_kwargs)
...
def create_parser():
    ...
    p['gpu_ids']=  list(range(1)) if p['gpu_ids'] is None else [int(t) for t in p['gpu_ids'].split(',')]
...
def print_random(num=10000):
    ...
    print(random.randint(1,num),np.random.randint(1,num),jittor.randint(num, (1,1)))
