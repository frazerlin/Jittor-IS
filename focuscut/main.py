...
class Trainer(object):
    def __init__(self,p):
        ...
        self.train_loader = DataLoader(self.train_set, batch_size=p['batch_size'], shuffle=True, num_workers=p['num_workers'])
        ...
        if len(p['gpu_ids'])>1:
            ...
            self.model = (jittor.nn.DataParallel(self.model, device_ids = p['gpu_ids']))
        ...
        if p['resume'] is not None:
            self.model.load_state_dict(jittor.load(p['resume']))
    ...
    def training(self, epoch): 
        ...
        tbar = tqdm(self.train_loader,total=len(self.train_loader)//self.p['batch_size'])
        ...
        for i, sample_batched in enumerate(tbar):
            ...
            if self.p['seq_mode']:
                ...
                if self.p['model'].startswith('hrcnet'):
                    output,loss_items, losses_sum=self.model(sample_batched)
            ...
            self.optimizer.step(losses_sum)
    ...
    def validation_robot(self, epoch,if_hrv=False):
        ...
        for index, val_robot_set in enumerate(self.val_robot_sets):
            ...
            for i in tqdm(range(len(val_robot_set))):
                sample = val_robot_set[i]
                id = sample['meta']['id']
...
if __name__ == "__main__": 
    ...
    jittor.misc.set_global_seed(p['seed'])
    ...
    if p['val']:
        ...
        for epoch in range(p['epochs']):
            ...
            # lr_str = '{:.7f}'.format(mine.scheduler.get_lr(mine.scheduler.base_lr, 0))
            # lr_str = ['{:.7f}'.format(i) for i in [mine.scheduler.get_lr(), mine.scheduler.get_lr()]]
            ...
            if (epoch+1) % p['val_robot_interval']==0:
                ...
                if p['save_val_robot']=='each': jittor.save(mine.model_src.state_dict(), '{}/model-epoch-{}.pth'.format(p['snapshot_path'],str(epoch).zfill(3)))
                ...
                if p['save_val_robot']=='best' and (mine.best_metric is None or current_metric[0]<mine.best_metric[0]):
                    ...
                    jittor.save(mine.model_src.state_dict(), '{}/best.pth'.format(p['snapshot_path']))
