...
import jittor.transform as jtr
########################################[ General ]########################################
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            ...
            tmp = jittor.array(tmp)
            if self.div_list is None:
                # tmp = tmp.float().div(255) if isinstance(tmp, torch.ByteTensor) else tmp.float()
                tmp = tmp.float().divide(255) if isinstance(tmp, jittor.uint8) else tmp.float()
            else:
                tmp = tmp.float().divide(255) if elem in self.div_list else tmp.float()
            sample[elem] = tmp                          
        return sample
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            ...
            for i in range(tensor.shape[0]):
                tensor[i] = (tensor[i] - self.mean_std[0][i]) / self.mean_std[1][i]
        # print(sample)
        # exit(0)
        return sample
...
class IIS(object):
    ...
    def __call__(self, sample):
        ...
        # print(sample['img'].sum())
        


        sample=self.Normalize(self.ToTensor(sample))

        # print(sample['img'].numpy().sum())

        # print(sample['gt'].numpy().sum())

        # print(sample['pos_points_mask'].numpy().sum())
        # print(sample['neg_points_mask'].numpy().sum())
        ...
        # print(sample['pos_points_mask_hr'].numpy().sum())
        # print(sample['neg_points_mask_hr'].numpy().sum())
