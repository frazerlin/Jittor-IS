...
class ISModel_prevMod(nn.Module):
    ...
    def execute(self, image, points):
        ...
        if jt.all(prev_mask == jt.zeros_like(prev_mask)):
            prev_masks = jt.concat([prev_mask, prev_mask], dim=1)
            ...
            outputs['instances'] = nn.interpolate(outputs['instances'], size=image.size()[2:], mode='bilinear', align_corners=True)
            prev_mask = jt.sigmoid(outputs['instances'])

        prev_mask_modulated = modulate_prevMask(prev_mask, points, self.N, self.R_max)
        
        prev_masks = jt.concat([prev_mask, prev_mask_modulated], dim=1)
    ...
    def get_coord_features(self, image, prev_masks, points):
        ...
        if prev_masks is not None:
            coord_features = jt.concat((prev_masks, coord_features), dim=1)
...
def split_points_by_order(tpoints: jt.Var, groups):
