    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        if not validation:
            with jt.enable_grad():
                ...
                image, gt_mask, points, prev_mask = batch_data['images'], batch_data['instances'], batch_data['points'], \
                    batch_data['prev_mask']
                orig_image, orig_gt_mask, orig_points = image.clone(), gt_mask.clone(), points.clone()
                click_indices = points[:, :, -1].max(dim=1) + 1
                net_input = jt.concat((image, prev_mask), dim=1) if self.net.with_prev_mask else image

                output = self.net(net_input, points)

                with jt.no_grad():
                    batch_data['next_prev_mask'] = jt.sigmoid(output['instances'])
                    batch_data['next_points'] = get_next_points(jt.sigmoid(output['instances']).clone(), orig_gt_mask,
                                                                points, click_indices)
                ...
                assert not jt.any(jt.isnan(loss))
