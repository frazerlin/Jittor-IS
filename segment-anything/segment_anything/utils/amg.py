...
def isclose(a: jt.Var, b: jt.Var, rtol=1e-05, atol=1e-8):
    return jt.array(jt.abs(a - b) <= rtol * jt.abs(b) + atol)
...
def is_box_near_crop_edge(
    ...
    crop_box_torch = jt.array(crop_box, dtype=jt.float)
    orig_box_torch = jt.array(orig_box, dtype=jt.float)
...
def calculate_stability_score(
    ...
    intersections = (
        (masks > (mask_threshold + threshold_offset)).cast(jt.int32)
        .sum(dim=-1).cast(jt.int32)
        .sum(dim=-1)
    )
    unions = (
        (masks > (mask_threshold - threshold_offset)).cast(jt.int32)
